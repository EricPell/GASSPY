...
def data_path(root,field,N,zero_padding=5):
    f = "%s/cube_%s_%0__x__i.fits".replace("__x__",str(zero_padding))
    return ("f"%(root,field,N))


append_db = defaults.append_db
try:
    append_db.update(myconfig.append_db)
except:pass

mask_parameters_dict = defaults.mask_parameters_dict
try:
    mask_parameters_dict.update(myconfig.mask_parameters_dict)
except:pass
...
try:
    compression_ratio.update(myconfig.compression_ratio)
except:pass
